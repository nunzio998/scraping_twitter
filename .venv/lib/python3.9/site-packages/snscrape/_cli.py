...
#import ntscraper.base
#import ntscraper.modules
#import ntscraper.version
import sys
import tempfile
		...
		for i, (module, frameRecord) in enumerate(zip(modules, trace)):
			if module is None:
				...
				for j in reversed(range(i)):
					...
					continue
				module = modules[j]
			if not module.__name__.startswith('ntscraper.') and module.__name__ != 'ntscraper':
				continue
...
class CitationAction(argparse.Action):
	...
	def __call__(self, parser, namespace, values, optionString):
		try:
			m = importlib.metadata.metadata('ntscraper')
		except importlib.metadata.PackageNotFoundError:
			print('Error: could not find ntscraper installation. --citation does not work without the package being installed.', file = sys.stderr)
			parser.exit(1)
...
def parse_args():
	...
	parser.add_argument('--version', action = 'version', version = f'ntscraper {snscrape.version.__version__}')
